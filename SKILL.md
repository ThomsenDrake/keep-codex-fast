...
4. Before applying cleanup, identify large/old active repo chats that may still matter. For each one the user wants to continue, create or update:
   - a repo-local handoff doc
   - a reactivation prompt that can start a fresh chat without losing the thread
5. If the user wants cleanup, ask them to close Codex or use `--wait-for-codex-exit`, then run:
